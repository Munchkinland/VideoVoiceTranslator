...
def texto_a_voz(texto, lang="en"):
    try:
        ...
        # Selecciona la primera voz disponible (puedes ajustar esto)
...
def generar_srt(subtitulos, archivo_srt):
    try:
        with open(archivo_srt, "w") as f:
            for i, (inicio, fin, texto) in enumerate(subtitulos):
                f.write(f"{i + 1}\n")
                f.write(f"{inicio} --> {fin}\n")
                f.write(f"{texto}\n\n")
    except Exception as e:
        print(f"Error al generar el archivo SRT: {e}")
...
def main(video_path):
    ...
    if video_final_path:
        ...
        # Aquí puedes definir los subtítulos de ejemplo; deberás ajustarlos según tus necesidades
        subtitulos = [
            ("00:00:01,000", "00:00:10,000", "Hola, este es un subtítulo de ejemplo."),
            ("00:00:10,000", "00:00:20,000", "Este es otro subtítulo de ejemplo.")
        ]
        generar_srt(subtitulos, "subtitulos.srt")
        print("Archivo de subtítulos SRT generado en: subtitulos.srt")
    else:
        print("Error en la generación del video final")

if __name__ == "__main__":
    video_path = "VideoVoiceTranslator/Lab_obs_lnd.mp4"  # Cambia esta ruta por la ruta de tu video
    main(video_path)
